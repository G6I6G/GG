...
class Game {
  constructor(audio, storage) {
    ...
    this.totalClicks = 0;
    this.sugarLumps = 0;
    this.sugarTimer = performance.now();
    ...
    this.nextGolden = performance.now() + 15000;
  ...
  createBuildings() {
    return [
      new Building({ id: "cursor", name: "Cursor", baseCost: 15, baseCps: 0.1 }),
      new Building({ id: "grandma", name: "Grandma", baseCost: 100, baseCps: 1 }),
      new Building({ id: "farm", name: "Farm", baseCost: 500, baseCps: 4 }),
      new Building({ id: "mine", name: "Mine", baseCost: 1200, baseCps: 10 }),
      new Building({ id: "factory", name: "Factory", baseCost: 3000, baseCps: 40 }),
      new Building({ id: "bank", name: "Bank", baseCost: 15000, baseCps: 120 }),
      new Building({ id: "temple", name: "Temple", baseCost: 20000, baseCps: 250 }),
      new Building({ id: "wizard", name: "Wizard Tower", baseCost: 33000, baseCps: 400 }),
      new Building({ id: "shipment", name: "Shipment", baseCost: 51000, baseCps: 800 }),
      new Building({ id: "alchemy", name: "Alchemy Lab", baseCost: 75000, baseCps: 1500 }),
      new Building({ id: "portal", name: "Portal", baseCost: 100000, baseCps: 3000 }),
      new Building({ id: "time", name: "Time Machine", baseCost: 123456, baseCps: 7000 }),
      new Building({ id: "antimatter", name: "Antimatter Condenser", baseCost: 250000, baseCps: 12000 }),
      new Building({ id: "prism", name: "Prism", baseCost: 800000, baseCps: 25000 }),
      new Building({ id: "chancemaker", name: "Chancemaker", baseCost: 1500000, baseCps: 60000 }),
      new Building({ id: "fractal", name: "Fractal Engine", baseCost: 2500000, baseCps: 120000 }),
      new Building({ id: "idleverse", name: "Idleverse", baseCost: 4500000, baseCps: 250000 }),
      new Building({ id: "you", name: "You", baseCost: 9000000, baseCps: 500000 }),
      new Building({ id: "cosmic", name: "Cosmic Oven", baseCost: 18000000, baseCps: 900000 }),
      new Building({ id: "mythical", name: "Mythical Bakery", baseCost: 36000000, baseCps: 1600000 }),
    ];
  }
  ...
  generateAchievements() {
    ...
    const clickThresholds = Array.from({ length: 150 }, (_, i) => 50 + i * 250);
    ...
    const cookieThresholds = Array.from({ length: 150 }, (_, i) => Math.pow(1.22, i) * 500);
    ...
    const upgradeThresholds = Array.from({ length: 120 }, (_, i) => (i + 1) * 5);
  ...
  addSugar(delta) {
    ...
    if (now - this.sugarTimer > 60000) {
      const gained = Math.floor((now - this.sugarTimer) / 60000);
      this.sugarLumps += gained;
      this.sugarTimer = now;
    }
  }
  ...
  buyBuilding(id) {
    ...
    if (!building) return;
    const qty = this.getPurchaseQuantity(building);
    let totalCost = 0;
    for (let i = 0; i < qty; i++) {
      totalCost += Math.round(building.baseCost * Math.pow(1.15, building.count + i));
    }
    if (this.purchaseMode === "max") {
      while (this.cookies >= Math.round(building.baseCost * Math.pow(1.15, building.count + 1))) {
        const c = Math.round(building.baseCost * Math.pow(1.15, building.count));
        if (this.cookies < c) break;
        this.cookies -= c;
        building.count += 1;
      }
    } else if (this.cookies >= totalCost) {
  ...
  getPurchaseQuantity(building) {
    if (this.purchaseMode === "max") return 0;
    const value = parseInt(this.purchaseMode, 10);
  ...
  spawnGoldenCookie() {
    const now = performance.now();
    if (!this.nextGolden || now < this.nextGolden) return;
    const goldenBtn = document.getElementById("golden-cookie");
    goldenBtn?.classList.remove("hidden");
    goldenBtn?.classList.add("flash");
    setTimeout(() => goldenBtn?.classList.remove("flash"), 600);
    this.nextGolden = now + (20000 + Math.random() * 20000);
    this.goldenExpires = now + 8000;
  }

  clickGolden() {
    const goldenBtn = document.getElementById("golden-cookie");
    goldenBtn?.classList.add("hidden");
    const roll = Math.random();
    if (roll < 0.4) {
      const bonus = this.getCps() * 30;
      this.addCookies(bonus);
      this.ui?.setMiniStatus(`Golden Cookie: +${formatNumber(bonus)} печенья!`);
    } else if (roll < 0.8) {
      this.addTemporaryClickBoost(7, 15);
      this.ui?.setMiniStatus("Golden Cookie: x7 клики на 15 сек.");
    } else {
      this.globalMultiplier *= 1.5;
      setTimeout(() => (this.globalMultiplier /= 1.5), 15000);
      this.ui?.setMiniStatus("Golden Cookie: +50% к производству на 15 сек.");
    }
  }
  ...
  loop() {
    ...
    if (this.goldenExpires && performance.now() > this.goldenExpires) {
      document.getElementById("golden-cookie")?.classList.add("hidden");
      this.goldenExpires = 0;
    }
...
class UI {
  constructor(game, audio, storage, miniGames) {
    ...
    this.purchaseModes = document.querySelectorAll(".purchase-modes .mode");
  ...
  bindEvents() {
    ...
    document.getElementById("toggle-theme").addEventListener("click", (e) => {
      ...
      e.currentTarget.textContent = `Тема: ${next === "light" ? "светлая" : "тёмная"}`;
    });
    document.getElementById("toggle-animations").addEventListener("click", (e) => {
      this.game.disableAnimations = !this.game.disableAnimations;
      document.body.dataset.animations = this.game.disableAnimations ? "off" : "on";
      e.currentTarget.textContent = `Анимации: ${this.game.disableAnimations ? "выкл" : "вкл"}`;
    });
    ...
    document.getElementById("golden-cookie").addEventListener("click", () => this.game.clickGolden());
    ...
    this.purchaseModes.forEach((mode) => {
      mode.addEventListener("click", () => {
        this.purchaseModes.forEach((m) => m.classList.remove("active"));
        mode.classList.add("active");
        this.game.purchaseMode = mode.dataset.qty;
      });
    });
    document.getElementById("season-select").addEventListener("change", (e) => {
      document.body.dataset.season = e.target.value;
    });
  }
  ...
  render() {
    ...
    document.getElementById("sugar-lumps").textContent = formatNumber(this.game.sugarLumps);
  ...
  updateBuildingButtons() {
    this.game.buildings.forEach((b) => {
      ...
      btn?.closest(".card")?.classList.toggle("affordable", this.game.cookies >= b.getCost());
    });
  }
  ...
  updateUpgradeButtons() {
    this.upgradeContainer.querySelectorAll("[data-up]").forEach((btn) => {
      ...
      if (upgrade) {
        const affordable = this.game.cookies >= upgrade.cost;
        btn.disabled = !affordable;
        btn.closest(".card")?.classList.toggle("affordable", affordable);
      }
    });
  }
  ...
  renderMiniGames() {
    ...
    this.enableTilt(this.miniContainer.querySelectorAll(".tilt"), 8);
  }
  ...
  updateGolden() {
    const timer = document.getElementById("golden-timer");
    if (this.game.nextGolden) {
      const diff = Math.max(0, this.game.nextGolden - performance.now());
      timer.textContent = `Golden через ${Math.ceil(diff / 1000)}с`;
    } else {
      timer.textContent = "";
    }
  }
  ...
  enableTilt(elements, strength = 10) {
    elements.forEach((el) => {
  ...
  enablePanelTilt() {
    ...
    this.enableTilt(panels, 4);
    this.enableTilt(document.querySelectorAll(".tilt"), 10);
    const cookie = document.getElementById("big-cookie");
    cookie.addEventListener("pointermove", (e) => {
      const rect = cookie.getBoundingClientRect();
      const dx = e.clientX - (rect.left + rect.width / 2);
      const dy = e.clientY - (rect.top + rect.height / 2);
      cookie.style.transform = `rotateX(${(-dy / rect.height) * 12}deg) rotateY(${(dx / rect.width) * 12}deg) scale(1.02)`;
    });
    cookie.addEventListener("pointerleave", () => {
